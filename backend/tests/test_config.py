...
def test_settings_public_config():
    ...
    # Verify Phase 9 is ready and active
    phase9 = public_config["pipeline_stages"][8]
    assert phase9["stage_id"] == "browser_automation_staging"
    assert phase9["status"] == "ready"
    assert phase9["active"] is True

    # Verify Phase 10 is ready and active
    phase10 = public_config["pipeline_stages"][9]
    assert phase10["stage_id"] == "portal_adapters_staging"
    assert phase10["status"] == "ready"
    assert phase10["active"] is True

    # Verify Phase 11 is ready and active
    phase11 = public_config["pipeline_stages"][10]
    assert phase11["stage_id"] == "hardening_resilience_security"
