...
class GreenhousePreparationAdapter(BasePortalPreparationAdapter):
    ...
    async def prepare(self, page: Any, context: PreparationContext) -> PreparationResult:
        ...
        has_app_container = (
            ...
            or await page.locator("#first_name, input[name='first_name'], input[id*='first_name'], input[name='name']").count() > 0
        )
        ...
        if await PlaywrightSafetyGuard.detect_captcha(page):
            logger.info(f"[GreenhouseAdapter] CAPTCHA detected on Greenhouse form for App #{context.application_id}. Pausing after pre-populating fields.")
