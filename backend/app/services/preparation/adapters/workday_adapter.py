...
class WorkdayPreparationAdapter(BasePortalPreparationAdapter):
    ...
    async def prepare(self, page: Any, context: PreparationContext) -> PreparationResult:
        ...
        # Check for blocking full-page challenge / auth wall before form exists (Scenario B)
        ...
        if await PlaywrightSafetyGuard.detect_captcha(page):
            logger.info(f"[WorkdayAdapter] CAPTCHA detected on Workday form for App #{context.application_id}. Pausing after pre-populating fields.")
