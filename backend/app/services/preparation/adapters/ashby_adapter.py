...
class AshbyPreparationAdapter(BasePortalPreparationAdapter):
    ...
    async def prepare(self, page: Any, context: PreparationContext) -> PreparationResult:
        ...
        has_ashby_form = (
            ...
            and await page.locator("input[name*='name'], input[name*='email'], input[type='email']").count() > 0
        )
        ...
        if await PlaywrightSafetyGuard.detect_captcha(page):
            logger.info(f"[AshbyAdapter] CAPTCHA detected on Ashby form for App #{context.application_id}. Pausing after pre-populating fields.")
