...
class GenericPortalPreparationAdapter(BasePortalPreparationAdapter):
    ...
    async def prepare(self, page: Any, context: PreparationContext) -> PreparationResult:
        ...
        # 2. Populate All Supported Master Profile & Screening Fields (Scenario A)
        fields_filled, unresolved_fields, resume_uploaded = await self.fill_common_form_fields(page, context)

        # 3. CAPTCHA / Bot Challenge Verification Check on Form
        if await PlaywrightSafetyGuard.detect_captcha(page):
            logger.info(f"[GenericAdapter] CAPTCHA detected on generic form for App #{context.application_id}. Pausing after pre-populating fields.")
        ...
        # 4. Non-Negotiable Submit Guard Check
        guard_triggered = await self.verify_submit_guard(page)

        # 5. Capture Staging Screenshot
